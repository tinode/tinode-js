 ...
 *  }
 */
'use strict'; // Regular expressions for parsing inline formats. Javascript does not support lookbehind,
// so it's a bit messy.

var INLINE_STYLES = [// Strong = bold, *bold text*
{
  name: "ST",
  start: /(?:^|\W)(\*)[^\s*]/,
  end: /[^\s*](\*)(?=$|\W)/
...
var ENTITY_TYPES = [// URLs
{
  name: "LN",
  dataName: "url",
  pack: function pack(val) {
...
var DECORATORS = {
  ...
  IM: {
    open: function open(data) {
      ...
      var res = (data.name ? '<a href="' + downloadUrl + '" download="' + data.name + '">' : '') + '<img src="' + previewUrl + '"' + (data.width ? ' width="' + data.width + '"' : '') + (data.height ? ' height="' + data.height + '"' : '') + ' border="0" />';
...
var Drafty = function Drafty() {}; // Take a string and defined earlier style spans, re-compose them into a tree where each leaf is
...
function chunkify(line, start, end, spans) {
  ...
  for (var i in spans) {
    ...
    var span = spans[i]; // Grab the initial unstyled chunk
...
function forEach(line, start, end, spans, formatter, context) {
  ...
  for (var i = 0; i < spans.length; i++) {
    var span = spans[i]; // Add un-styled range before the styled span starts.
    ...
    result.push(formatter.call(context, span.tp, span.data, tag.isVoid ? null : forEach(line, start, span.at + span.len, subspans, formatter, context)));
...
function spannify(original, re_start, re_end, type) {
  ...
  while (line.length > 0) {
    ...
    var start_offset = start['index'] + start[0].lastIndexOf(start[1]); // Clip the processed part of the string.

    line = line.slice(start_offset + 1); // start_offset is an offset within the clipped string. Convert to original index.

    start_offset += index; // Index now point to the beginning of 'line' within the 'original' string.

    index = start_offset + 1; // Find the matching closing token.
    ...
    var end_offset = end['index'] + end[0].indexOf(end[1]); // Clip the processed part of the string.

    line = line.slice(end_offset + 1); // Update offsets

    end_offset += index; // Index now point to the beginning of 'line' within the 'original' string.
...
Drafty.parse = function (content) {
  ...
  var entityMap = [];
  var entityIndex = {}; // Processing lines one by one, hold intermediate result in blx.

  var blx = [];
  lines.map(function (line) {
    var spans = [];
    var entities = []; // Find formatted spans in the string.
    ...
    if (spans.length == 0) {
      ...
      spans = toTree(spans); // Build a tree representation of the entire string, not
...
Drafty.attachFile = function (content, mime, base64bits, fname, size, refurl) {
  ...
  if (refurl instanceof Promise) {
    ex.data.ref = refurl.then(function (url) {
      ex.data.ref = url;
    }, function (err) {
...
Drafty.format = function (content, formatter, context) {
  ...
  var spans = [].concat(fmt); // Zero values may have been stripped. Restore them.
...
var VERSION = package_version || "0.15";
var LIBRARY = "tinodejs/" + VERSION;
var TOPIC_NEW = "new";
var TOPIC_ME = "me";
var TOPIC_FND = "fnd";
var USER_NEW = "new"; // Starting value of a locally-generated seqId used for pending messages.

var LOCAL_SEQID = 0xFFFFFFF;
var MESSAGE_STATUS_NONE = 0; // Status not assigned.

var MESSAGE_STATUS_QUEUED = 1; // Local ID assigned, in progress to be sent.

var MESSAGE_STATUS_SENDING = 2; // Transmission started.

var MESSAGE_STATUS_SENT = 3; // Delivered to the server.

var MESSAGE_STATUS_RECEIVED = 4; // Received by the client.

var MESSAGE_STATUS_READ = 5; // Read by the user.

var MESSAGE_STATUS_TO_ME = 6; // Message from another user.
// Utility functions
...
function mergeObj(dst, src, ignore) {
  ...
  for (var prop in src) {
    if (src.hasOwnProperty(prop) && (src[prop] || src[prop] === false) && (!ignore || !ignore[prop]) && prop != '_generated') {
...
function xdreq() {
  var xdreq = null; // Detect browser support for CORS
...
function jsonBuildHelper(key, val) {
  ...
  } else if (val === undefined || val === null || val === false || Array.isArray(val) && val.length == 0 || _typeof(val) === "object" && Object.keys(val).length === 0) {
...
function simplify(obj) {
  Object.keys(obj).forEach(function (key) {
    if (key[0] == "_") {
      ...
      // Strip fields which evaluate to false.
      delete obj[key];
    } else if (_typeof(obj[key]) == 'object' && !(obj[key] instanceof Date)) {
      simplify(obj[key]); // Strip empty objects.
...
function jsonParseHelper(key, val) {
  ...
  } else if (key === 'acs' && _typeof(val) === 'object') {
...
function getBrowserInfo(ua) {
  ...
  if (m) {
    ...
    var tokens = []; // Split Name/0.0.0 into Name and version 0.0.0
...
var CBuffer = function CBuffer(compare) {
  var buffer = [];
  ...
  return {
     ...
     */
    put: function put() {
      var insert; // inspect arguments: if array, insert its elements, if one or more non-array arguments, insert them one by one
     ...
     */
    delRange: function delRange(since, before) {
     ...
     */
    forEach: function forEach(callback, startIdx, beforeIdx, context) {
      startIdx = startIdx | 0;
     ...
     */
    find: function find(elem, nearest) {
...
var Connection = function Connection(host_, apiKey_, transport_, secure_, autoreconnect_) {
  var _this = this;

  var host = host_;
  var secure = secure_;
  var apiKey = apiKey_;
  var autoreconnect = autoreconnect_; // Settings for exponential backoff

  var _BOFF_BASE = 2000; // 2000 milliseconds, minimum delay between reconnects

  var _BOFF_MAX_ITER = 10; // Maximum delay between reconnects 2^10 * 2000 ~ 34 minutes

  var _BOFF_JITTER = 0.3; // Add random delay
  ...
  function reconnect() {
    ...
    // Clear timer
    clearTimeout(_boffTimer); // Calculate when to fire the reconnect attempt

    var timeout = _BOFF_BASE * (Math.pow(2, _boffIteration) * (1.0 + _BOFF_JITTER * Math.random())); // Update iteration counter for future use


    _boffIteration = _boffIteration >= _BOFF_MAX_ITER ? _boffIteration : _boffIteration + 1;
    _boffTimer = setTimeout(function () {
      log("Reconnecting, iter=" + _boffIteration + ", timeout=" + timeout); // Maybe the socket was closed while we waited for the timer?
  ...
  if (transport_ === "lp") {
    ...
    if ((typeof window === "undefined" ? "undefined" : _typeof(window)) != 'object' || !window["WebSocket"]) {
...
var Tinode = function Tinode(appname_, host_, apiKey_, transport_, secure_) {
  var _this3 = this;
  ...
  this._apiKey = apiKey_; // Name and version of the browser.

  this._browser = '';
  this._platform = "undefined";
  this._humanLanguage = 'xx'; // Underlying OS.
  ...
  this._loggingEnabled = false; // When logging, trip long strings (base64-encoded images) for readability

  this._trimLongStrings = false; // UID of the currently authenticated user.

  this._myUID = null; // Status of connection: authenticated or not.

  this._authenticated = false; // Login used in the last successful basic authentication

  this._login = null; // Token which can be used for login instead of login/password.

  this._authToken = null; // Counter of received packets

  this._inPacketCount = 0; // Counter for generating unique message IDs

  this._messageId = Math.floor(Math.random() * 0xFFFF + 0xFFFF); // Information about the server, if connected

  this._serverInfo = null; // Push notification token. Called deviceToken for consistency with the Android SDK.

  this._deviceToken = null; // Cache of pending promises

  this._pendingPromises = {};
  /** A connection object, see {@link Connection}. */

  this._connection = new Connection(host_, apiKey_, transport_, secure_, true); // Console logger

  this.logger = function (str) {
    if (_this3._loggingEnabled) {
      var d = new Date();
      var dateString = ('0' + d.getUTCHours()).slice(-2) + ':' + ('0' + d.getUTCMinutes()).slice(-2) + ':' + ('0' + d.getUTCSeconds()).slice(-2) + ':' + ('0' + d.getUTCMilliseconds()).slice(-3);
      console.log('[' + dateString + '] ' + str);
    }
  };

  this._connection.logger = this.logger; // Tinode's cache of objects

  this._cache = {};

  var cachePut = this.cachePut = function (type, name, obj) {
    _this3._cache[type + ":" + name] = obj;
  };

  var cacheGet = this.cacheGet = function (type, name) {
    return _this3._cache[type + ":" + name];
  };

  var cacheDel = this.cacheDel = function (type, name) {
    delete _this3._cache[type + ":" + name];
  }; // Enumerate all items in cache, call func for each item.
  // Enumeration stops if func returns true.


  var cacheMap = this.cacheMap = function (func, context) {
    for (var idx in _this3._cache) {
      if (func(_this3._cache[idx], idx, context)) {
        break;
      }
    }
  ...
  this.attachCacheToTopic = function (topic) {
    topic._tinode = _this3;
  ...
  var execPromise = function execPromise(id, code, onOK, errorText) {
    var callbacks = _this3._pendingPromises[id];

    if (callbacks) {
      delete _this3._pendingPromises[id];
  ...
  var getNextUniqueId = this.getNextUniqueId = function () {
    return _this3._messageId != 0 ? '' + _this3._messageId++ : undefined;
  }; // Get User Agent string


  var getUserAgent = function getUserAgent() {
    return _this3._appName + " (" + (_this3._browser ? _this3._browser + "; " : "") + _this3._platform + "); " + LIBRARY;
  }; // Generator of packets stubs


  this.initPacket = function (type, topic) {
    var pkt = null;
      ...
      case "hi":
        return {
          "hi": {
            ...
            "ver": VERSION,
            "ua": getUserAgent(),
            "dev": _this3._deviceToken,
            "lang": _this3._humanLanguage
          }
        };
      ...
      case "get":
        return {
          "get": {
            ...
            "topic": topic,
            "what": null,
            // data, sub, desc, space separated list; unknown strings are ignored
            "desc": {},
            "sub": {},
            "data": {}
          }
  ...
  this.send = function (pkt, id) {
    ...
    _this3.logger("out: " + (_this3._trimLongStrings ? JSON.stringify(pkt, jsonLoggerHelper) : msg));

    try {
      _this3._connection.sendText(msg);
    } catch (err) {
  ...
  this.loginSuccessful = function (ctrl) {
    ...
    _this3._myUID = ctrl.params.user;
    _this3._authenticated = ctrl && ctrl.code >= 200 && ctrl.code < 300;
    ...
    if (ctrl.params && ctrl.params.token && ctrl.params.expires) {
      ...
      _this3._authToken = null;
    }

    if (_this3.onLogin) {
      _this3.onLogin(ctrl.code, ctrl.text);
    }
  ...
  this._connection.onMessage = function (data) {
    ...
    if (!data) return;
    _this3._inPacketCount++; // Send raw message to listener

    if (_this3.onRawMessage) {
      _this3.onRawMessage(data);
    }

    var pkt = JSON.parse(data, jsonParseHelper);

    if (!pkt) {
      _this3.logger("in: " + data);

      _this3.logger("ERROR: failed to parse data");
    } else {
      _this3.logger("in: " + (_this3._trimLongStrings ? JSON.stringify(pkt, jsonLoggerHelper) : data)); // Send complete packet to listener


      if (_this3.onMessage) {
        _this3.onMessage(pkt);
      }

      if (pkt.ctrl) {
        // Handling {ctrl} message
        if (_this3.onCtrlMessage) {
          _this3.onCtrlMessage(pkt.ctrl);
        ...
        if (_this3.onMetaMessage) {
          _this3.onMetaMessage(pkt.meta);
        }
      } else if (pkt.data) {
        ...
        if (_this3.onDataMessage) {
          _this3.onDataMessage(pkt.data);
        }
      } else if (pkt.pres) {
        ...
        if (_this3.onPresMessage) {
          _this3.onPresMessage(pkt.pres);
        }
      } else if (pkt.info) {
        ...
        if (_this3.onInfoMessage) {
          _this3.onInfoMessage(pkt.info);
        }
      } else {
        _this3.logger("ERROR: Unknown packet received.");
      }
    }
  }; // Ready to send.
  ...
  this._connection.onDisconnect = function (err) {
    _this3._inPacketCount = 0;
    _this3._serverInfo = null;
    _this3._authenticated = false;
...
Tinode.MESSAGE_STATUS_NONE = MESSAGE_STATUS_NONE, Tinode.MESSAGE_STATUS_QUEUED = MESSAGE_STATUS_QUEUED, Tinode.MESSAGE_STATUS_SENDING = MESSAGE_STATUS_SENDING, Tinode.MESSAGE_STATUS_SENT = MESSAGE_STATUS_SENT, Tinode.MESSAGE_STATUS_RECEIVED = MESSAGE_STATUS_RECEIVED, Tinode.MESSAGE_STATUS_READ = MESSAGE_STATUS_READ, Tinode.MESSAGE_STATUS_TO_ME = MESSAGE_STATUS_TO_ME, // Unicode [del] symbol.
Tinode.DEL_CHAR = "\u2421"; // Public methods;
...
Tinode.prototype = {
   ...
   */
  connect: function connect(host_) {
  ...
  */
   ...
   */
  account: function account(uid, scheme, secret, login, params) {
    var pkt = this.initPacket("acc");
    pkt.acc.user = uid;
    pkt.acc.scheme = scheme;
    pkt.acc.secret = secret; // Log in to the new account using selected scheme
   ...
   */
  createAccount: function createAccount(scheme, secret, login, params) {
    var _this4 = this;
   ...
   * @returns {Promise} Promise which will be resolved/rejected when server reply is received.
   */
  createAccountBasic: function createAccountBasic(username, password, params) {
   ...
   * @returns {Promise} Promise which will be resolved/rejected when server reply is received.
   */
  updateAccountBasic: function updateAccountBasic(uid, username, password, params) {
    // Make sure we are not using 'null' or 'undefined';
    username = username || '';
    password = password || '';
  ...
  hello: function hello() {
    ...
    return this.send(pkt, pkt.hi.id).then(function (ctrl) {
      ...
      if (_this5.onDisconnect) {
        _this5.onDisconnect(err);
      }
    });
  },
   ...
   */
  setDeviceToken: function setDeviceToken(dt, sendToServer) {
    var sent = false;
   ...
   * @returns {Promise} Promise which will be resolved/rejected when server reply is received.
   */
  login: function login(scheme, secret, cred) {
    var _this6 = this;
    ...
    pkt.login.cred = cred;
    return this.send(pkt, pkt.login.id).then(function (ctrl) {
      _this6.loginSuccessful(ctrl);
   ...
   */
  loginBasic: function loginBasic(uname, password, cred) {
    var _this7 = this;

    return this.login("basic", b64EncodeUnicode(uname + ":" + password), cred).then(function (ctrl) {
      _this7._login = uname;
      return ctrl;
    });
  },
   ...
   * @returns {Promise} Promise which will be resolved/rejected on receiving server reply.
   */
  loginToken: function loginToken(token, cred) {
   ...
   */
  requestResetAuthSecret: function requestResetAuthSecret(scheme, method, value) {
   ...
   * @returns {Promise} Promise which will be resolved/rejected on receiving server reply.
   */
  subscribe: function subscribe(topicName, getParams, setParams) {
   ...
   * @returns {Promise} Promise which will be resolved/rejected on receiving server reply.
   */
  leave: function leave(topic, unsub) {
   ...
   */
  createMessage: function createMessage(topic, data, noEcho) {
    var pkt = this.initPacket("pub", topic);
    var dft = typeof data == "string" ? Drafty.parse(data) : data;
   ...
   * @returns {Promise} Promise which will be resolved/rejected on receiving server reply.
   */
  publish: function publish(topic, data, noEcho) {
   ...
   * @returns {Promise} Promise which will be resolved/rejected on receiving server reply.
   */
  publishMessage: function publishMessage(pub) {
   ...
   * @returns {Promise} Promise which will be resolved/rejected on receiving server reply.
   */
  getMeta: function getMeta(topic, params) {
   ...
   * @returns {Promise} Promise which will be resolved/rejected on receiving server reply.
   */
  setMeta: function setMeta(topic, params) {
   ...
   * @returns {Promise} Promise which will be resolved/rejected on receiving server reply.
   */
  delMessages: function delMessages(topic, ranges, hard) {
  ...
  delTopic: function delTopic(topic) {
    ...
    var pkt = this.initPacket("del", topic);
    pkt.del.what = "topic";
    return this.send(pkt, pkt.del.id).then(function (ctrl) {
      _this8.cacheDel("topic", topic);
   ...
   * @returns {Promise} Promise which will be resolved/rejected on receiving server reply.
   */
  delSubscription: function delSubscription(topic, user) {
    var pkt = this.initPacket("del", topic);
    pkt.del.what = "sub";
    pkt.del.user = user;
    return this.send(pkt, pkt.del.id);
   ...
   */
  note: function note(topic, what, seq) {
   ...
   */
  noteKeyPress: function noteKeyPress(topic) {
    var pkt = this.initPacket("note", topic);
    pkt.note.what = "kp";
    this.send(pkt);
  },
   ...
   */
  getTopic: function getTopic(name) {
   ...
   */
  newTopic: function newTopic(callbacks) {
   ...
   */
  newGroupTopicName: function newGroupTopicName() {
   ...
   * @returns {Tinode.Topic} Newly created topic.
   */
  newTopicWith: function newTopicWith(peer, callbacks) {
   ...
   */
  getMeTopic: function getMeTopic() {
   ...
   */
  getFndTopic: function getFndTopic() {
   ...
   */
  getLargeFileHelper: function getLargeFileHelper() {
   ...
   */
  getCurrentLogin: function getCurrentLogin() {
    return this._login;
  },
   ...
   */
  enableLogging: function enableLogging(enabled, trimLongStrings) {
   ...
   */
  isTopicOnline: function isTopicOnline(name) {
   ...
   * @deprecated
   */
  wantAkn: function wantAkn(status) {
    if (status) {
...
var MetaGetBuilder = function MetaGetBuilder(parent) {
  this.topic = parent;
...
MetaGetBuilder.prototype = {
  ...
  _get_ims: function _get_ims() {
    var cupd = this.contact && this.contact.updated;
    var tupd = this.topic._lastDescUpdate || 0;
   ...
   */
  withData: function withData(since, before, limit) {
    this.what["data"] = {
      since: since,
      before: before,
      limit: limit
   ...
   * @returns {Tinode.MetaGetBuilder} <tt>this</tt> object.
   */
  withLaterData: function withLaterData(limit) {
   ...
   * @returns {Tinode.MetaGetBuilder} <tt>this</tt> object.
   */
  withEarlierData: function withEarlierData(limit) {
   ...
   * @returns {Tinode.MetaGetBuilder} <tt>this</tt> object.
   */
  withDesc: function withDesc(ims) {
    this.what["desc"] = {
      ims: ims
    };
    return this;
   ...
   * @returns {Tinode.MetaGetBuilder} <tt>this</tt> object.
   */
  withLaterDesc: function withLaterDesc() {
   ...
   * @returns {Tinode.MetaGetBuilder} <tt>this</tt> object.
   */
  withSub: function withSub(ims, limit, userOrTopic) {
    var opts = {
      ims: ims,
      limit: limit
    };
   ...
   * @returns {Tinode.MetaGetBuilder} <tt>this</tt> object.
   */
  withOneSub: function withOneSub(ims, userOrTopic) {
   ...
   * @returns {Tinode.MetaGetBuilder} <tt>this</tt> object.
   */
  withLaterOneSub: function withLaterOneSub(userOrTopic) {
   ...
   * @returns {Tinode.MetaGetBuilder} <tt>this</tt> object.
   */
  withLaterSub: function withLaterSub(limit) {
    return this.withSub(this.topic.getType() == "p2p" ? this._get_ims() : this.topic._lastSubsUpdate, limit);
  },
   ...
   * @returns {Tinode.MetaGetBuilder} <tt>this</tt> object.
   */
  withDel: function withDel(since, limit) {
    if (since || limit) {
      this.what["del"] = {
        since: since,
        limit: limit
   ...
   * @returns {Tinode.MetaGetBuilder} <tt>this</tt> object.
   */
  withLaterDel: function withLaterDel(limit) {
...
var AccessMode = function AccessMode(acs) {
  if (acs) {
...
AccessMode.update = function (val, upd) {
  ...
  if (action == '+' || action == '-') {
    var val0 = val; // Split delta-string like '+ABC-DEF+Z' into an array of parts including + and -.

    var parts = upd.split(/([-+])/); // Starting iteration from 1 because String.split() creates an array with the first empty element.
...
AccessMode.prototype = {
   ...
   * @returns {AccessMode} - <b>this</b> AccessMode.
   */
  updateMode: function updateMode(u) {
   ...
   * @returns {AccessMode} - <b>this</b> AccessMode.
   */
  updateGiven: function updateGiven(u) {
   ...
   * @returns {AccessMode} - <b>this</b> AccessMode.
   */
  updateWant: function updateWant(u) {
   ...
   * @returns {string} - <b>given</b> value.
   */
  getWant: function getWant() {
    return AccessMode.encode(this.want);
  },
  updateAll: function updateAll(val) {
    if (val) {
  ...
  isPresencer: function isPresencer() {
    return (this.mode & AccessMode._PRES) != 0;
  },
  isJoiner: function isJoiner() {
  ...
  isSharer: function isSharer() {
    return (this.mode & AccessMode._SHARE) != 0;
  },
  isDeleter: function isDeleter() {
...
var Topic = function Topic(name, callbacks) {
  // Parent Tinode object.
  this._tinode = null; // Server-provided data, locally immutable.
  // topic name

  this.name = name; // timestamp when the topic was created

  this.created = null; // timestamp when the topic was last updated

  this.updated = null; // timestamp of the last messages

  this.touched = null; // access mode, see AccessMode

  this.acs = new AccessMode(null); // per-topic private data

  this.private = null; // per-topic public data

  this.public = null; // Locally cached data
  // Subscribed users, for tracking read/recv/msg notifications.

  this._users = {}; // Current value of locally issued seqId, used for pending messages.

  this._queuedSeqId = LOCAL_SEQID; // The maximum known {data.seq} value.

  this._maxSeq = 0; // The minimum known {data.seq} value.

  this._minSeq = 0; // Indicator that the last request for earlier messages returned 0.

  this._noEarlierMsgs = false; // The maximum known deletion ID.

  this._maxDel = 0; // User discovery tags

  this._tags = []; // Message cache, sorted by message seq values, from old to new.

  this._messages = CBuffer(function (a, b) {
    return a.seq - b.seq;
  }); // Boolean, true if the topic is currently live

  this._subscribed = false; // Timestap when topic meta-desc update was recived.

  this._lastDescUpdate = null; // Timestap when topic meta-subs update was recived.

  this._lastSubsUpdate = null; // Used only during initialization

  this._new = true; // Callbacks

  if (callbacks) {
    this.onData = callbacks.onData;
    this.onMeta = callbacks.onMeta;
    this.onPres = callbacks.onPres;
    this.onInfo = callbacks.onInfo; // A single desc update;

    this.onMetaDesc = callbacks.onMetaDesc; // A single subscription record;

    this.onMetaSub = callbacks.onMetaSub; // All subscription records received;
...
Topic.prototype = {
   ...
   */
  subscribe: function subscribe(getParams, setParams) {
    var _this9 = this;
    ...
    var name = this.name; // Send subscribe message, handle async response.
    ...
    return this._tinode.subscribe(name || TOPIC_NEW, getParams, setParams).then(function (ctrl) {
      ...
      _this9._subscribed = true;
      _this9.acs = ctrl.params && ctrl.params.acs ? ctrl.params.acs : _this9.acs; // Set topic name for new topics and add it to cache.

      if (_this9._new) {
        _this9._new = false;
        _this9.name = ctrl.topic;
        _this9.created = ctrl.ts;
        _this9.updated = ctrl.ts;
        _this9.touched = ctrl.ts;

        _this9._cachePutSelf(); // Add the new topic to the list of contacts maintained by the 'me' topic.


        var me = _this9._tinode.getMeTopic();
        ...
        if (setParams && setParams.desc) {
          ...
          _this9._processMetaDesc(setParams.desc);
        }
      }
   ...
   * @returns {Promise} Promise to be resolved/rejected when the server responds to the request.
   */
  publish: function publish(data, noEcho) {
   ...
   */
  createMessage: function createMessage(data, noEcho) {
  ...
  publishMessage: function publishMessage(pub) {
    ...
    if (Drafty.hasAttachments(pub.content)) {
      var attachments = [];
      Drafty.attachments(pub.content, function (data) {
   ...
   */
  publishDraft: function publishDraft(pub, prom) {
    var _this10 = this;
    ...
    pub._generated = true;
    pub.ts = new Date();
    pub.from = this._tinode.getCurrentUserID(); // Don't need an echo message becasue the message is added to local cache right away.

    pub.noecho = true; // Add to cache.
    ...
    prom = (prom || Promise.resolve()).then(function ()
    /* argument ignored */
    {
      if (pub._cancelled) {
        return {
      ...
      return _this10.publishMessage(pub).then(function (ctrl) {
        pub._sending = false;
        pub.seq = ctrl.params.seq;
        pub.ts = ctrl.ts;

        _this10._routeData(pub);
      ...
      _this10._messages.delAt(_this10._messages.find(pub));
  ...
  leave: function leave(unsub) {
    ...
    return this._tinode.leave(this.name, unsub).then(function (ctrl) {
      _this11._resetSub();
   ...
   */
  getMeta: function getMeta(params) {
    if (!this._subscribed) {
   ...
   */
  getMessagesPage: function getMessagesPage(limit, forward) {
    var _this12 = this;
   ...
   * @returns {Promise} Promise to be resolved/rejected when the server responds to request.
   */
  setMeta: function setMeta(params) {
    var _this13 = this;
    ...
    return this._tinode.setMeta(this.name, params).then(function (ctrl) {
      ...
      if (params.sub) {
        ...
        if (!params.sub.user) {
          ...
          params.sub.user = _this13._tinode.getCurrentUserID();
        ...
        _this13._processMetaSub([params.sub]);
      }
      ...
      if (params.desc) {
        ...
        _this13._processMetaDesc(params.desc);
      }

      if (params.tags) {
        _this13._processMetaTags(params.tags);
      }
   ...
   * @returns {Promise} Promise to be resolved/rejected when the server responds to request.
   */
  invite: function invite(uid, mode) {
    return this.setMeta({
      sub: {
        user: uid,
        mode: mode
   ...
   * @returns {Promise} Promise to be resolved/rejected when the server responds to request.
   */
  delMessages: function delMessages(ranges, hard) {
    var _this14 = this;
    ...
    var tosend = ranges.reduce(function (out, r) {
    ...
    return result.then(function (ctrl) {
      if (ctrl.params.del > _this14._maxDel) {
        _this14._maxDel = ctrl.params.del;
      }

      ranges.map(function (r) {
        if (r.hi) {
          _this14.flushMessageRange(r.low, r.hi);
        } else {
          _this14.flushMessage(r.low);
        }
      });
   ...
   * @returns {Promise} Promise to be resolved/rejected when the server responds to request.
   */
  delMessagesAll: function delMessagesAll(hardDel) {
   ...
   * @returns {Promise} Promise to be resolved/rejected when the server responds to request.
   */
  delMessagesList: function delMessagesList(list, hardDel) {
    // Sort the list in ascending order
    list.sort(function (a, b) {
      return a - b;
    }); // Convert the array of IDs to ranges.

    var ranges = list.reduce(function (out, id) {
      if (out.length == 0) {
        // First element.
        out.push({
          low: id
   ...
   * @returns {Promise} Promise to be resolved/rejected when the server responds to request.
   */
  delSubscription: function delSubscription(user) {
    var _this15 = this;
    ...
    return this._tinode.delSubscription(this.name, user).then(function (ctrl) {
      // Remove the object from the subscription cache;
      delete _this15._users[user]; // Notify listeners

      if (_this15.onSubsUpdated) {
        _this15.onSubsUpdated(Object.keys(_this15._users));
      }
   ...
   */
  noteRecv: function noteRecv(seq) {
    this.note("recv", seq);
  },
   ...
   * @param {Number} seq - ID of the message to aknowledge.
   */
  noteRead: function noteRead(seq) {
    this.note("read", seq);
  },
   ...
   * @memberof Tinode.Topic#
   */
  noteKeyPress: function noteKeyPress() {
    if (this._subscribed) {
   ...
   */
  userDesc: function userDesc(uid) {
   ...
   */
  subscribers: function subscribers(callback, context) {
   ...
   */
  subscriber: function subscriber(uid) {
   ...
   * @param {Object} context - Value of `this` inside the `callback`.
   */
  messages: function messages(callback, sinceId, beforeId, context) {
    var cb = callback || this.onData;

    if (cb) {
      var startIdx = typeof sinceId == 'number' ? this._messages.find({
        seq: sinceId
      }) : undefined;
      var beforeIdx = typeof beforeId == 'number' ? this._messages.find({
        seq: beforeId
      }, true) : undefined;
   ...
   * @param {Number} seq - ID or the message read or received.
   */
  msgReceiptCount: function msgReceiptCount(what, seq) {
    var count = 0;
   ...
   */
  msgReadCount: function msgReadCount(seq) {
   ...
   * @returns {number} Number of subscribers who claim to have received the message.
   */
  msgRecvCount: function msgRecvCount(seq) {
   ...
   */
  msgHasMoreMessages: function msgHasMoreMessages(newer) {
    return newer ? this.seq > this._maxSeq : // _minSeq cound be more than 1, but earlier messages could have been deleted.
   ...
   */
  isNewMessage: function isNewMessage(seqId) {
   ...
   */
  flushMessage: function flushMessage(seqId) {
    var idx = this._messages.find({
      seq: seqId
    });
   ...
   */
  flushMessageRange: function flushMessageRange(fromId, untilId) {
    // start: find exact match.
    // end: find insertion point (nearest == true).
    var since = this._messages.find({
      seq: fromId
    });
   ...
   */
  cancelSend: function cancelSend(seqId) {
    var idx = this._messages.find({
      seq: seqId
    });

    if (idx >= 0) {
      var msg = this._messages.getAt(idx);

      var status = this.msgStatus(msg);
   ...
   */
  getAccessMode: function getAccessMode() {
    return this.acs;
  },
   ...
   */
  getDefaultAccess: function getDefaultAccess() {
    return this.defacs;
  },
   ...
   */
  startMetaQuery: function startMetaQuery() {
   ...
   */
  msgStatus: function msgStatus(msg) {
  ...
  _routeData: function _routeData(data) {
  ...
  _routeMeta: function _routeMeta(meta) {
    if (meta.desc) {
  ...
  _routePres: function _routePres(pres) {
    var user;
    ...
    switch (pres.what) {
      ...
      case "acs":
        var uid = pres.src == "me" ? this._tinode.getCurrentUserID() : pres.src;
  ...
  _routeInfo: function _routeInfo(info) {
    if (info.what !== "kp") {
  ...
  _processMetaDesc: function _processMetaDesc(desc, fromMe) {
  ...
  _processMetaSub: function _processMetaSub(subs) {
  ...
  _processMetaTags: function _processMetaTags(tags) {
  ...
  _processDelMessages: function _processDelMessages(clear, delseq) {
  ...
  _allMessagesReceived: function _allMessagesReceived(count) {
    if (this.onAllMessagesReceived) {
      this.onAllMessagesReceived(count);
    }
  },
  // Reset subscribed state
  _resetSub: function _resetSub() {
    this._subscribed = false;
  },
  // This topic is either deleted or unsubscribed from.
  _gone: function _gone() {
  ...
  _updateCachedUser: function _updateCachedUser(uid, obj, requestUpdate) {
    ...
    this._cachePutUser(uid, cached); // Save to the list of topic subsribers.


    return mergeToCache(this._users, uid, cached);
  },
  // Get local seqId for a queued message.
  _getQueuedSeqId: function _getQueuedSeqId() {
...
var TopicMe = function TopicMe(callbacks) {
  Topic.call(this, TOPIC_ME, callbacks); // List of contacts (topic_name -> Contact object)

  this._contacts = {}; // me-specific callbacks
...
TopicMe.prototype = Object.create(Topic.prototype, {
  ...
  _processMetaSub: {
    value: function value(subs) {
      var updateCount = 0;

      for (var idx in subs) {
        var sub = subs[idx];
        var topicName = sub.topic; // Don't show 'me' and 'fnd' topics in the list of contacts.
        ...
        sub.deleted = sub.deleted ? new Date(sub.deleted) : null; // Ensure the values are integer.
    ...
    value: function value(pres) {
      ...
      if (cont) {
        ...
        if (!acs || acs.mode == AccessMode._INVALID) {
          ...
          this.getMeta(this.startMetaQuery().withOneSub(undefined, pres.src).build()); // Create a dummy entry to catch online status update.
   ...
   */
  setMsgReadRecv: {
    value: function value(contactName, what, seq, ts) {
...
var TopicFnd = function TopicFnd(callbacks) {
  Topic.call(this, TOPIC_FND, callbacks); // List of users and topics uid or topic_name -> Contact object)
...
TopicFnd.prototype = Object.create(Topic.prototype, {
  ...
  _processMetaSub: {
    value: function value(subs) {
      var updateCount = Object.getOwnPropertyNames(this._contacts).length; // Reset contact list.
...
var LargeFileHelper = function LargeFileHelper(tinode) {
  this._tinode = tinode;
  this._apiKey = tinode._apiKey;
  this._authToken = tinode.getAuthToken();
  this._msgId = tinode.getNextUniqueId();
  this.xhr = xdreq(); // Promise

  this.toResolve = null;
  this.toReject = null; // Callbacks
...
LargeFileHelper.prototype = {
   ...
   */
  upload: function upload(file, onProgress, onSuccess, onFailure) {
    var _this16 = this;
    ...
    var result = new Promise(function (resolve, reject) {
      _this16.toResolve = resolve;
      _this16.toReject = reject;
    });
   ...
   */
  download: function download(relativeUrl, filename, mimetype, onProgress) {
    var _this17 = this;
    ...
    var instance = this; // Get data as blob (stored by the browser as a temporary file).
    ...
    var result = new Promise(function (resolve, reject) {
      _this17.toResolve = resolve;
      _this17.toReject = reject;
...
var Message = function Message(topic_, content_) {
...
},{"../version.json":3,"./drafty.es5.js":1}],3:[function(require,module,exports){
